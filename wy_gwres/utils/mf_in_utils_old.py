...
def make_model(data_dict=None, sim=None,tdis=None,gwf=None,ims=None,dis=None,ic=None,
               npf=None,chd=None,well=None,evt=None,drn=None,
...
def make_chd(chd_dict=None,save_flows=True,default_head=1.,active_layers=[0]):
    ...
    if 'lake_data' in list(chd_dict.keys()):
        ...
        if 'timeseries' not in list(lake_data.keys()):
            
            lake_xyv = list(zip(inds[:,1],inds[:,2],len(inds[:,1])*[lake_data['start_head']]))
        
            chd_spd_dict['in_dict'] = {'xyv':lake_xyv}
            chd_rec = prep_spd(**chd_spd_dict)
        else:
            # Use time series to set lake water level
            
            # Make full stress period data record
